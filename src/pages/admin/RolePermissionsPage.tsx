...
export default function RolePermissionsPage() {
    ...
    const [permissionRecord, setPermissionRecord] = useState<PermissionRecord>({});
    const [activeTypePermissions, setActiveTypePermissions] = useState<RolePermission[]>([]);
    ...
    const loadPermissions = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            setPermissionRecord(nextPermissionRecord);
            setActiveTypePermissions(nextPermissionsByType);
        } catch (nextError: any) {
            setError(nextError?.message || 'Failed to load role permissions.');
            setPermissionRecord({});
            setActiveTypePermissions([]);
        } finally {
            setLoading(false);
        }
    ...
    useEffect(() => {
        if (selectedRole === 'Public' && activeTab !== 'feature') {
    ...
    const persistPermission = useCallback(async (
        ...
        return true;
    }, [activeTab, isAdminView, loadPermissions, selectedRole, setLocalPermissionState, showToast]);
    ...
    const handleResetToDefaults = useCallback(async () => {
        ...
        setIsResetting(true);
        const success = await permissionService.resetRoleToDefaults(selectedRole);
        ...
        <div className="min-h-screen bg-slate-100">
                                                ...
                                                <div className="flex flex-shrink-0 items-center gap-2">
                                                    <button
                                                        ...
                                                        className={`relative inline-flex h-8 w-14 items-center rounded-full transition ${
                                                            isToggleEnabled ? 'bg-[#14b8a6]' : 'bg-[#cbd5e1]'
                                                        } ${
                                                    ...
                                                    >
                                                        <span
                                                            className={`inline-flex h-6 w-6 items-center justify-center rounded-full bg-white shadow transition ${
                                                                isToggleEnabled ? 'translate-x-7' : 'translate-x-1'
                                                            }`}
                                                        >
                                                            {isSaving ? (
            ...
            {toast && (
                <div className="fixed right-4 top-4 z-50 w-full max-w-sm">
                    <div className={`rounded-2xl border px-4 py-3 shadow-xl ${
                        toast.type === 'success'
