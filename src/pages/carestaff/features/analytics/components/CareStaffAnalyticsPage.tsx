...
const EMPTY_ANALYTICS_DATA = { submissions: [], answers: [] };
...
const ViewAnswersModal = ({ student, questions, answers, onClose }: any) => (
            <m.div
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-6"
            >
                <button type="button" aria-label="Close response details" className="absolute inset-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-purple-400" onClick={onClose} />
                <m.div
                ...
                >
                    ...
                    <div className="px-8 py-7 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-purple-50 to-violet-50/50">
                        <div>
                            <h3 className="font-black text-xl text-slate-900">
                                {student.students?.first_name}&apos;s Response
                            </h3>
                    ...
                    <div className="flex-1 overflow-y-auto px-8 py-6 space-y-5">
                        {questions.map((q, idx) => {
                            const answer = answers.find((a: any) => a.submission_id === student.id && a.question_id === q.id);
                            return (
                                <m.div
                                    key={q.id}
...
const RespondentsTab = ({ courseFilter, setCourseFilter, uniqueCourses, respondents, sortConfig, onSort, onViewStudent }: any) => (
                <m.div
                ...
                >
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                            ...
                            <span className="text-sm font-bold text-slate-600">{respondents.length} students</span>
                        </div>
                    </div>
                            ...
                            <thead className="bg-slate-50/80 border-b border-slate-200/60 text-[10px] uppercase text-slate-500 font-black tracking-widest">
                                <tr>
                                    <th scope="col" aria-sort={sortConfig.key === 'name' ? (sortConfig.direction === 'asc' ? 'ascending' : 'descending') : 'none'} className="p-0">
                                        <button type="button" className="w-full cursor-pointer px-7 py-5 text-left hover:bg-slate-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-purple-500" onClick={() => onSort('name')}>
                                    ...
                                    <th scope="col" aria-sort={sortConfig.key === 'course' ? (sortConfig.direction === 'asc' ? 'ascending' : 'descending') : 'none'} className="p-0">
                                        <button type="button" className="w-full cursor-pointer px-7 py-5 text-left hover:bg-slate-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-purple-500" onClick={() => onSort('course')}>
                                    ...
                                    <th scope="col" aria-sort={sortConfig.key === 'date' ? (sortConfig.direction === 'asc' ? 'ascending' : 'descending') : 'none'} className="p-0">
                                        <button type="button" className="w-full cursor-pointer px-7 py-5 text-left hover:bg-slate-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-purple-500" onClick={() => onSort('date')}>
...
const OverviewTab = ({ submissions, answers, questions, topQuestionScoreFilter, setTopQuestionScoreFilter }: any) => (
                <m.div
                ...
                >
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                        ...
                        >
                            ...
                            <TopQuestionsChart questions={questions} answers={answers} scoreFilter={topQuestionScoreFilter} />
                        </m.div>
                    </div>
                    ...
                    >
                        ...
                        <div className="p-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {questions.map((q) => (
                                <QuestionChart key={q.id} question={q} answers={answers} />
                            ))}
                            {questions.length === 0 && (
...
const AnalyticsKpiCards = ({ stats, prevStats, isComparisonMode }: any) => (
...
const AnalyticsHeader = ({
    ...
    dateFilter, setFormSelection, isComparisonMode, onComparisonToggle,
    ...
    <m.div variants={fadeUp} className="bg-white/80 backdrop-blur-xl rounded-[2rem] border border-white/60 shadow-xl shadow-purple-500/5 ring-1 ring-slate-200/50 p-8">
        ...
        <div className="flex flex-wrap items-end gap-5 p-5 bg-slate-50/70 rounded-2xl border border-slate-200/60">
            <div className="flex flex-col gap-1.5">
                <label htmlFor="care-analytics-period-start" className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Period Start</label>
                <input
                    id="care-analytics-period-start"
                    type="date"
            ...
            <div className="flex flex-col gap-1.5">
                <label htmlFor="care-analytics-period-end" className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Period End</label>
                <input
                    id="care-analytics-period-end"
                    type="date"
...
const fetchAnalyticsSubmissions = async (selectedFormId: number | null) => {
    ...
    const studentIds = [...new Set(subs.flatMap(s => s.student_id ? [s.student_id] : []))];
...
const CareStaffAnalyticsPage = ({ functions }: CareStaffAnalyticsPageProps) => {
    ...
    const analyticsData = qAnalyticsData || EMPTY_ANALYTICS_DATA;

    const [comparisonLoading, setComparisonLoading] = useState(false);
    const queriesLoading = !qForms || !qDepartments || (!!selectedFormId && (analyticsLoading || questionsLoading));
    const loading = queriesLoading || comparisonLoading;
    ...
    const { filteredData, stats } = useMemo(() => {
        ...
        return {
            filteredData: { submissions: subs, answers: ans },
            stats: processAnalyticsData(subs)
        };
    ...
    const sortedRespondents = sortRespondents(filteredData.submissions, sortConfig);

    const uniqueCourses = [...new Set(analyticsData.submissions.flatMap((s: any) => s.students?.course ? [s.students.course] : []))] as string[];
    ...
    return (
        <m.div variants={stagger} initial="hidden" animate="show" className="space-y-8 pb-10">
            ...
            {/* ── KPI BENTO CARDS ── */}
            <AnalyticsKpiCards stats={stats} prevStats={prevStats} isComparisonMode={isComparisonMode} />
