...
const EvaluationFormModal = ({ evaluation, printRef, criteriaLabels, onPrint, onClose }: any) => (
                    ...
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <div className="mt-3 inline-flex flex-col gap-0.5 rounded-lg bg-white/10 px-3 py-2">
                                ...
                                <p className="text-base font-extrabold text-white">{evaluation.events?.title || 'Untitled event'}</p>
                                {formatEventSchedule(evaluation.events) && (
                                    <p className="text-[11px] font-semibold text-blue-200">{formatEventSchedule(evaluation.events)}</p>
                                )}
                            </div>
                        </div>
                        <Button variant="ghost" size="sm" onClick={onClose} className="!bg-white/15 hover:!bg-white/25 !text-white !rounded-lg flex-shrink-0">✕</Button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-5 lg:p-6" ref={printRef}>
                    <div style={{ display: 'none' }} className="eval-header">
                        <div className="eval-header">
                            ...
                            <p style={{ fontWeight: 700 }}>{evaluation.events?.title || 'Untitled event'}</p>
                            {formatEventSchedule(evaluation.events) && <p>{formatEventSchedule(evaluation.events)}</p>}
                        </div>
                    </div>
                        ...
                        <div style={{ border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                                <div style={{ padding: '10px 16px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', color: '#6b7280' }}>Criteria</div>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: '#6b7280' }}>Score</div>
                            </div>
                            {criteriaLabels.map((label, idx) => {
                                const score = evaluation[`q${idx + 1}_score`];
                    ...
                    {evaluation.open_best && (
                                <div className="open-section"><p style={{ display: 'block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: '#6b7280', marginBottom: '4px' }}>What they liked best about the activity</p><p style={{ background: '#eff6ff', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', margin: 0 }}>{evaluation.open_best}</p></div>
                    )}
                    {evaluation.open_suggestions && (
                                <div className="open-section"><p style={{ display: 'block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: '#6b7280', marginBottom: '4px' }}>Suggestions to further improve</p><p style={{ background: '#eff6ff', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', margin: 0 }}>{evaluation.open_suggestions}</p></div>
                    )}
                    {evaluation.open_comments && (
                                <div className="open-section"><p style={{ display: 'block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: '#6b7280', marginBottom: '4px' }}>Other comments</p><p style={{ background: '#eff6ff', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', margin: 0 }}>{evaluation.open_comments}</p></div>
                    )}
                </div>
...
const CsmFeedbackModal = ({ feedback, ccQuestions, getCcAnswerText, onClose }: any) => (
    <div className="fixed inset-x-0 bottom-3 top-3 z-[70] flex items-start justify-center overflow-visible bg-transparent px-3 md:top-[4.75rem]">
                    ...
                    <div className="space-y-4">
                        <div className="rounded-xl border border-gray-200 bg-white p-4">
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                                <div><p className="text-[10px] font-bold text-gray-400 uppercase">Client Type</p><p className="text-sm font-bold">{feedback.client_type || '-'}</p></div>
                                <div><p className="text-[10px] font-bold text-gray-400 uppercase">Sex</p><p className="text-sm font-bold">{feedback.sex || '-'}</p></div>
                                <div><p className="text-[10px] font-bold text-gray-400 uppercase">Age</p><p className="text-sm font-bold">{feedback.age || '-'}</p></div>
                        ...
                        <div className="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
                            ...
                            <div className="mt-3 space-y-2">
                                {ccQuestions.map((item: any) => {
                                    const rawValue = feedback[item.key];
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="overflow-hidden rounded-xl border border-gray-200">
                                ...
                                {SQD_LABELS.map((label, idx) => {
                                    const score = feedback[`sqd${idx}`];
                        ...
                        {feedback.suggestions && (
                            <div>
                                ...
                                <p className="rounded-lg bg-blue-50 p-3 text-sm leading-relaxed text-gray-700">{feedback.suggestions}</p>
                            </div>
                        )}
                    </div>
                </div>
        ...
        <div className="space-y-6">
            ...
            {viewingCSM && (
                <CsmFeedbackModal
                    ...
                    getCcAnswerText={getCcAnswerText}
                    onClose={() => setViewingCSM(null)}
                />
            )}
        </div>
    );
