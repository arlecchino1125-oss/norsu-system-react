...
/** Read-only profile modal (registrar-safe categories) with full-size photo viewer. */
const RegistrarProfileModal = ({
    student, isLoading, categoryIndex, setCategoryIndex, showPhotoModal, setShowPhotoModal, onClose
}: any) => (
    <>
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <button type="button" aria-label="Close student profile" className="absolute inset-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal-400" onClick={() => onClose()} />
            <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-teal-50 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-4">
                        <button type="button"
                            onClick={() => student?.profile_picture_url && setShowPhotoModal(true)}
                            className={`w-14 h-14 rounded-xl overflow-hidden bg-gradient-to-br from-teal-500 to-cyan-400 flex items-center justify-center text-2xl font-black text-white shrink-0 shadow-lg shadow-teal-200 ${student?.profile_picture_url ? 'cursor-pointer hover:opacity-90 hover:ring-2 hover:ring-teal-400 transition-all focus:outline-none' : 'cursor-default'}`}
                        >
                            {student.profile_picture_url ? (
                                <ResolvedProfileImage storedValue={student.profile_picture_url} studentId={student.student_id} alt="Profile" className="w-full h-full object-cover" referrerPolicy="no-referrer" previewOnClick={false} />
                            ) : (
                                <span>{student.first_name?.[0] || 'S'}</span>
                            )}
                        </button>
                        <div>
                    ...
                    <button type="button" aria-label="Close student profile" onClick={() => onClose()} className="p-2 text-slate-400 hover:text-slate-600 bg-white rounded-full transition shadow-sm border border-slate-100">
                        <XCircle size={24} />
                    </button>
                </div>
                        ...
                        <div className="flex sm:flex-col gap-1 px-2">
                            {PROFILE_CATEGORIES.map((cat, i) => (
                                <button type="button"
                                    ...
                                    className={`text-left px-4 py-3 flex items-center gap-3 text-sm transition-all rounded-lg ${categoryIndex === i
                        ...
                        {isLoading ? (
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-6">
                                    <span className={`p-2 rounded-lg bg-gradient-to-br ${PROFILE_CATEGORIES[categoryIndex].gradient} text-white text-lg`}>
                                        {PROFILE_CATEGORIES[categoryIndex].icon}
                                    </span>
                                    <h3 className="text-xl font-bold text-slate-900">{PROFILE_CATEGORIES[categoryIndex].label}</h3>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {PROFILE_CATEGORIES[categoryIndex].fields.map((field: any, idx: number) => {
                                        let value = field.compute ? field.compute(student) : student[field.db];
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
            ...
            <div className="w-full lg:w-80 shrink-0 space-y-6">
                ...
                <select id="registrar-college-filter" value={departmentFilter} onChange={(e) => { setDepartmentFilter(e.target.value); setCourseFilter('All'); setCurrentPage(1); }} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white text-slate-700 truncate">
                        ...
                        <div>
                <label htmlFor="registrar-program-filter" className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Program</label>
                <select id="registrar-program-filter" value={courseFilter} onChange={(e) => { setCourseFilter(e.target.value); setCurrentPage(1); }} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white text-slate-700 truncate">
                        ...
                        <div>
                <label htmlFor="registrar-year-filter" className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Year Level</label>
                <select id="registrar-year-filter" value={yearFilter} onChange={(e) => { setYearFilter(e.target.value); setCurrentPage(1); }} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white text-slate-700">
                        ...
                        <div>
                <label htmlFor="registrar-status-filter" className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Status</label>
                <select id="registrar-status-filter" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:border-teal-500 bg-white text-slate-700">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex min-h-[28rem] flex-col">
                    ...
                    <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4 text-xs md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-center gap-1">
                            <button type="button" aria-label="Previous page" disabled={currentPage === 1 || loading} onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} className="px-2 py-1 border rounded bg-white hover:bg-slate-50 disabled:opacity-50"><ChevronLeft size={16} /></button>
                            <span className="px-3 font-medium text-slate-700">Page {currentPage} of {totalPages}</span>
                            <button type="button" aria-label="Next page" disabled={currentPage === totalPages || loading} onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} className="px-2 py-1 border rounded bg-white hover:bg-slate-50 disabled:opacity-50"><ChevronRight size={16} /></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {profileViewStudent && typeof document !== 'undefined' && createPortal(
                <RegistrarProfileModal
                    student={profileViewStudent}
                    isLoading={profileLoading}
                    categoryIndex={profileCategoryIndex}
                    setCategoryIndex={setProfileCategoryIndex}
