...
function useNatPortal() {
    ...
    const natOptionsQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            const requirements = Array.isArray(statsData?.requirements)
                ? statsData.requirements.flatMap((item: unknown) => {
    ...
    return {
        draftSnapshot, currentScreen, setCurrentScreen, currentStep, setCurrentStep, loading, setLoading, captchaRequired, setCaptchaRequired, captchaToken, setCaptchaToken, captchaResetKey, setCaptchaResetKey, submissionCaptchaToken, setSubmissionCaptchaToken, submissionCaptchaResetKey, setSubmissionCaptchaResetKey, showSuccessModal, setShowSuccessModal, showActivationModal, setShowActivationModal, activationConfirm, setActivationConfirm, activationSuccess, setActivationSuccess, currentUser, setCurrentUser, currentUserStatusRef, credentials, setCredentials, natSession, setNatSession, natBrowserId, natOptionsQuery, availableCourses, availableDates, natRequirements, supportsTestTime, formData, setFormData, showTimeInConfirm, setShowTimeInConfirm, showTimeOutConfirm, setShowTimeOutConfirm, elapsedTime, setElapsedTime, fieldErrors, setFieldErrors, validationScope, setValidationScope, isSessionSyncing, setIsSessionSyncing, lastSessionSyncAt, setLastSessionSyncAt, isPrivacyOpen, setIsPrivacyOpen, hasRestoredDraft, toast, setToast, showToast, natPortalAccessState, showNatPortalAvailability, supportsNatAttendance, hasStartedCurrentNat, hasFinishedCurrentNat, canTimeInCurrentNat, canTimeOutCurrentNat, selectedDateSchedule, selectedDateTimeSlots, runFormValidation, resolveInputClassName, updateFormDraft, shouldCheckNatTime, trustedNatTimeQuery, timeState, handleChange, handleCheckboxGroup, handlePrivacyAgreementChange, handleDobChange, handleSubmit, handleLogin, executeTimeIn, executeTimeOut, executeActivation, handleActivation, handleConfirmedActivation, handleActivationSuccessContinue, handleActivationSuccessLogin, handleNatLogout, isDark, toggleTheme, loginStudent, navigate, permissionsLoading, permissionsError, getFeatureAccessState
    };
}

const NatWizardScreen = ({
    setCurrentScreen, currentStep, setCurrentStep, loading, submissionCaptchaToken, setSubmissionCaptchaToken, submissionCaptchaResetKey, screenState, setShowSuccessModal, credentials, availableCourses, availableDates, supportsTestTime, formData, fieldErrors, setIsPrivacyOpen, toast, selectedDateTimeSlots, resolveInputClassName, handleChange, handlePrivacyAgreementChange, handleDobChange, handleSubmit, toggleTheme
}: any) => {
const { showSuccessModal, isPrivacyOpen, hasRestoredDraft, isDark } = screenState;
return (
<NATLayout
...
const NatActivationSuccessModal = ({ loading, activationSuccess, setActivationSuccess, handleActivationSuccessContinue, handleActivationSuccessLogin }: any) => (
...
const NatActivationConfirmModal = ({ loading, activationConfirm, setActivationConfirm, handleConfirmedActivation }: any) => (
...
const NatActivationFormModal = ({ loading, setShowActivationModal, availableCourses, handleActivation }: any) => (
    ...
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-white/20 relative">
        <div className="sticky top-0 z-10 bg-white rounded-t-2xl">
            ...
            <div className="flex justify-between items-start p-6 pb-4 pt-8">
                ...
                <button type="button" aria-label="Close account activation" onClick={() => setShowActivationModal(false)} className="bg-gray-100 p-2 rounded-full hover:bg-gray-200 transition-colors"><X className="w-5 h-5 text-gray-500" /></button>
            </div>
        </div>
...
const NatTimeOutConfirmModal = ({ loading, setShowTimeOutConfirm, supportsNatAttendance, executeTimeOut }: any) => (
...
const NatTimeInConfirmModal = ({ loading, setShowTimeInConfirm, supportsNatAttendance, executeTimeIn }: any) => (
<div className="fixed inset-0 bg-transparent flex items-center justify-center z-[200] p-4 animate-in fade-in duration-200">
    <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden border border-white/20">
...
const NatAttendanceWidget = ({ currentUser, elapsedTime, supportsNatAttendance, hasStartedCurrentNat, hasFinishedCurrentNat }: any) => (
...
const NatDashboardScreen = ({
    loading, screenState, setShowActivationModal, activationConfirm, setActivationConfirm, activationSuccess, setActivationSuccess, currentUser, availableCourses, natRequirements, setShowTimeInConfirm, setShowTimeOutConfirm, elapsedTime, lastSessionSyncAt, toast, timeState, executeTimeIn, executeTimeOut, handleActivation, handleConfirmedActivation, handleActivationSuccessContinue, handleActivationSuccessLogin, handleNatLogout, toggleTheme
}: any) => {
const {
    showActivationModal,
    showTimeInConfirm,
...
>
    <div className="nat-page-shell nat-page-shell-xl max-w-6xl mx-auto w-full animate-slide-in-up pb-24">
        ...
        {showTimeInConfirm && (
            <NatTimeInConfirmModal
                loading={loading}
                setShowTimeInConfirm={setShowTimeInConfirm}
                supportsNatAttendance={supportsNatAttendance}
                executeTimeIn={executeTimeIn}
            />
        )}

        {/* Time Out Confirmation Modal */}
        {showTimeOutConfirm && (
            <NatTimeOutConfirmModal
                loading={loading}
                setShowTimeOutConfirm={setShowTimeOutConfirm}
                supportsNatAttendance={supportsNatAttendance}
        ...
        {activationConfirm.visible && (
            <NatActivationConfirmModal
                loading={loading}
                activationConfirm={activationConfirm}
                setActivationConfirm={setActivationConfirm}
                handleConfirmedActivation={handleConfirmedActivation}
            />
        )}

        {/* Activation Success Modal */}
        {activationSuccess.visible && (
            <NatActivationSuccessModal
                loading={loading}
                activationSuccess={activationSuccess}
                setActivationSuccess={setActivationSuccess}
                handleActivationSuccessContinue={handleActivationSuccessContinue}
                handleActivationSuccessLogin={handleActivationSuccessLogin}
            />
        )}
        ...
        {hasStartedCurrentNat && (
            <NatAttendanceWidget
                ...
                supportsNatAttendance={supportsNatAttendance}
                hasStartedCurrentNat={hasStartedCurrentNat}
                hasFinishedCurrentNat={hasFinishedCurrentNat}
            />
        )}
                </div>
...
const NatLoginScreen = ({
    setCurrentScreen, loading, captchaRequired, captchaToken, setCaptchaToken, captchaResetKey, credentials, toast, handleLogin, isDark, toggleTheme
}: any) => (
<NATLayout
...
const NatWelcomeScreen = ({
    setCurrentScreen, credentials, isDark, toggleTheme, navigate
}: any) => (
<NATLayout
    isDark={isDark}
    onToggleTheme={toggleTheme}
...
const NATPortal = () => {
    ...
    if (currentScreen === 'dashboard') {
        ...
        return (
            <NatDashboardScreen
                ...
                currentUser={currentUser}
                availableCourses={availableCourses}
                natRequirements={natRequirements}
                setShowTimeInConfirm={setShowTimeInConfirm}
                setShowTimeOutConfirm={setShowTimeOutConfirm}
                elapsedTime={elapsedTime}
                lastSessionSyncAt={lastSessionSyncAt}
                toast={toast}
                timeState={timeState}
                executeTimeIn={executeTimeIn}
                executeTimeOut={executeTimeOut}
                ...
                handleActivationSuccessContinue={handleActivationSuccessContinue}
                handleActivationSuccessLogin={handleActivationSuccessLogin}
                handleNatLogout={handleNatLogout}
                toggleTheme={toggleTheme}
            />
        );
    }
    ...
    return (
        <NatWizardScreen
            ...
            loading={loading}
            submissionCaptchaToken={submissionCaptchaToken}
            setSubmissionCaptchaToken={setSubmissionCaptchaToken}
            submissionCaptchaResetKey={submissionCaptchaResetKey}
            screenState={{
                showSuccessModal,
                isPrivacyOpen,
                hasRestoredDraft,
