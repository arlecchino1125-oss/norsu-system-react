...
const DeptHomePage = ({
    ...
    const todayInterviews = useMemo(
        () => (Array.isArray(admissionApplicants) ? admissionApplicants : []).filter((app: any) => getDateKey(app?.interview_date) === todayKey),
        [admissionApplicants, todayKey]
    );

    const todayCounselingSessions = useMemo(
        () => (Array.isArray(counselingRequests) ? counselingRequests : []).filter((request: any) => (
            isCounselingCalendarVisible(String(request?.status || '').trim())
            && getDateKey(getCounselingScheduledDate(request)) === todayKey
        )),
        [counselingRequests, todayKey]
    );
    ...
    const primaryStats = [
        { label: 'Total Requests', value: filteredData.requests.length, icon: <FileText size={20} />, gradient: 'from-blue-400 to-indigo-500' },
        { label: 'Pending Approval', value: filteredData.requests.filter((request: any) => isCounselingAwaitingDept(request.status)).length, icon: <Info size={20} />, gradient: 'from-amber-400 to-orange-500' },
        { label: 'With CARE Staff', value: filteredData.requests.filter((request: any) => isWithCareStaffCounseling(request.status)).length, icon: <CheckCircle2 size={20} />, gradient: 'from-emerald-400 to-teal-500' },
        { label: 'Total Students', value: filteredData.students.length, icon: <Users size={20} />, gradient: 'from-violet-400 to-purple-500' }
    ];
                        ...
                        <div className="space-y-3">
                            <button onClick={() => setActiveModule('counseling_queue')} className={`card-hover w-full text-left p-4 rounded-xl bg-white border border-gray-100 hover:border-emerald-200 flex items-start gap-4 group ${FOCUS_RING}`}>
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{counselingRequests.filter((request: any) => isCounselingAwaitingDept(request.status)).length} pending review</p>
                                </div>
                            </button>
                            <button
                                onClick={() => {
