...
/** Read-only self-referral form with status-dependent actions. */
const CounselingViewModal = ({
    request, pendingCounselingCompletionId, onClose, onOpenSchedule, onOpenReject, onComplete, onForward
}: any) => (
            ...
            <div className="p-8">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-1">Submitted: {new Date(request.created_at).toLocaleString()}</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${isCounselingAwaitingDept(request.status) ? 'bg-amber-100 text-amber-700' : request.status === COUNSELING_STATUS.SCHEDULED ? 'bg-blue-100 text-blue-700' : request.status === COUNSELING_STATUS.STAFF_SCHEDULED ? 'bg-indigo-100 text-indigo-700' : request.status === COUNSELING_STATUS.REFERRED ? 'bg-purple-100 text-purple-700' : request.status === COUNSELING_STATUS.COMPLETED ? 'bg-green-100 text-green-700' : request.status === COUNSELING_STATUS.REJECTED ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>{isCounselingAwaitingDept(request.status) ? 'Pending Review' : request.status === COUNSELING_STATUS.STAFF_SCHEDULED ? 'With CARE Staff' : request.status}</span>
                        <button type="button" aria-label="Close counseling request" onClick={onClose} className="text-gray-400 hover:text-gray-600"><XCircle /></button>
                    </div>
                </div>
                ...
                <div className="mb-4">
                    <label htmlFor="dept-request-reason" className="block text-xs font-bold text-gray-500 mb-1">Reason/s for Requesting Counseling</label>
                    <textarea id="dept-request-reason" readOnly rows={4} value={request.reason_for_referral || request.description || ''} className="w-full bg-gray-100 border border-gray-200 rounded-xl p-4 text-sm text-gray-700 cursor-not-allowed resize-none"></textarea>
                </div>
                <div className="mb-4">
                    <label htmlFor="dept-request-actions" className="block text-xs font-bold text-gray-500 mb-1">Personal Actions Taken</label>
                    <textarea id="dept-request-actions" readOnly rows={3} value={request.personal_actions_taken || ''} className="w-full bg-gray-100 border border-gray-200 rounded-xl p-4 text-sm text-gray-700 cursor-not-allowed resize-none"></textarea>
                </div>
                <div className="mb-4">
                    <label htmlFor="dept-request-concern-duration" className="block text-xs font-bold text-gray-500 mb-1">Date / Duration of Concern</label>
                    <textarea id="dept-request-concern-duration" readOnly rows={2} value={request.date_duration_of_concern || ''} className="w-full bg-gray-100 border border-gray-200 rounded-xl p-4 text-sm text-gray-700 cursor-not-allowed resize-none"></textarea>
                </div>
                ...
                {request.referrer_signature && (
                    <div className="mb-4">
                        ...
                        <div className="bg-white border-2 border-dashed border-purple-200 rounded-xl p-4 flex flex-col items-center">
                            <img src={request.referrer_signature} alt="Referrer Signature" className="max-h-24 object-contain" />
                            <div className="w-48 border-t border-gray-400 mt-2 pt-1 text-center">
                                <p className="text-sm font-bold text-gray-800">{request.referred_by}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {getCounselingScheduledDate(request) && (
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-3"><p className="text-xs font-bold text-blue-800 uppercase mb-1">Scheduled Session</p><p className="text-sm font-semibold text-blue-900">{new Date(getCounselingScheduledDate(request) as string).toLocaleString()}</p></div>
                )}
                {request.referred_by && (
                    <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 mb-3"><p className="text-xs font-bold text-purple-800 uppercase mb-1">Forwarded by</p><p className="text-sm font-semibold text-purple-900">{request.referred_by}</p></div>
                )}
            </div>
            {/* Action buttons */}
            <div className="p-6 border-t border-gray-100 flex flex-wrap gap-3 sticky bottom-0 bg-white rounded-b-2xl">
                {isCounselingAwaitingDept(request.status) && (
                    <>
                        <button type="button" onClick={onOpenSchedule} className="flex-1 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-bold text-sm shadow-lg shadow-emerald-200/50 hover:shadow-xl transition-all">Approve & Schedule</button>
                        <button type="button" onClick={onOpenReject} className="flex-1 py-2.5 bg-red-50 text-red-700 border border-red-200 rounded-xl font-bold text-sm hover:bg-red-100 transition-all">Reject</button>
                    </>
                )}
                {request.status === COUNSELING_STATUS.SCHEDULED && (
                    <>
                        <button type="button" disabled={pendingCounselingCompletionId === String(request.id)} onClick={() => onComplete(request)} className="flex-1 py-2.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-bold text-sm shadow-lg shadow-green-200/50 hover:shadow-xl transition-all disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:shadow-lg">{pendingCounselingCompletionId === String(request.id) ? 'Completing...' : 'Mark as Completed'}</button>
                        <button type="button" onClick={() => onForward(request)} className="flex-1 py-2.5 bg-purple-50 text-purple-700 border border-purple-200 rounded-xl font-bold text-sm hover:bg-purple-100 transition-all">Forward to CARE Staff</button>
                    </>
                )}
                <button type="button" onClick={onClose} className="w-full py-2 text-gray-500 text-sm font-medium hover:text-gray-700 transition-colors">Close</button>
            </div>
        </div>
    </div>
);
...
const DeptCounselingQueuePage = ({
    ...
    const { showCounselingViewModal, showScheduleModal, showRejectModal } = modalState;
    const { isSubmittingCounselingSchedule, isSubmittingCounselingReject } = submissionState;
    ...
    const filteredRequests = useMemo(() => (
        counselingRequests.filter((request: any) => (
            matchesCounselingTab(request)
    ...
    const pageKey = `${counselingTab}:${filteredRequestSignature}`;
    const currentPage = pageState.key === pageKey ? Math.min(pageState.page, totalPages) : 1;
    ...
    const goToPage = (nextPage: number) => {
        ...
        setPageState({ key: pageKey, page: safePage });
    };
    ...
    return (
        <>
            ...
            {showCounselingViewModal && selectedCounselingReq && (
                <CounselingViewModal
                    request={selectedCounselingReq}
                    pendingCounselingCompletionId={pendingCounselingCompletionId}
                    onClose={() => setShowCounselingViewModal(false)}
                    onOpenSchedule={() => { setShowCounselingViewModal(false); setShowScheduleModal(true); }}
                    onOpenReject={() => { setShowCounselingViewModal(false); setShowRejectModal(true); }}
                ...
                <div className="fixed inset-0 bg-transparent z-[60] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button type="button" aria-label="Close scheduling dialog" onClick={() => setShowScheduleModal(false)} className="text-gray-400 hover:text-gray-600"><XCircle /></button>
                        </div>
                        <form onSubmit={handleApproveAndSchedule} className="p-6 space-y-4">
                            ...
                            <div><label htmlFor="dept-session-time" className="block text-xs font-bold text-gray-500 mb-1">Session Time <span className="text-red-400">*</span></label><input id="dept-session-time" type="time" value={scheduleData.time} onChange={e => setScheduleData({ ...scheduleData, time: e.target.value })} className="w-full px-4 py-2 border rounded-xl text-sm" required /></div>
                            <div><label htmlFor="dept-session-notes" className="block text-xs font-bold text-gray-500 mb-1">Notes (Optional)</label><textarea id="dept-session-notes" value={scheduleData.notes} onChange={e => setScheduleData({ ...scheduleData, notes: e.target.value })} className="w-full px-4 py-2 border rounded-xl text-sm h-20" placeholder="Additional instructions for the student..."></textarea></div>
                ...
                <div className="fixed inset-0 bg-transparent z-[60] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button type="button" aria-label="Close rejection dialog" onClick={() => setShowRejectModal(false)} className="text-gray-400 hover:text-gray-600"><XCircle /></button>
                        </div>
