...
const applyInterviewDateFilter = (rows: any[], interviewDateFilter?: string) => {
...
const getDepartmentAdmissionsPayload = async (
...
const getPrintableMasterListPayload = async (departmentName: unknown, printableMasterListDate: string, masterListPanelFilter: string): Promise<ExportPayload> => {
    ...
    const filteredRows = queueRows.filter((row: any) => (
...
const getEventsPayload = async (scope: string): Promise<ExportPayload> => {
    const { data, error } = await supabase
        .from('events')
...
const getCareNatPayload = async (): Promise<ExportPayload> => {
    const { data, error } = await supabase
        .from('applications')
...
const getCareCounselingPayload = async (): Promise<ExportPayload> => {
    const { data, error } = await supabase
        .from('counseling_requests')
...
const getCareSupportPayload = async (): Promise<ExportPayload> => {
    const { data, error } = await supabase
        .from('support_requests')
...
/** Interview-day master list card with its own date and panel filters. */
const PrintableMasterListCard = ({
    styles, dateValue, onDateChange, panelValue, onPanelChange, panelOptions, exportingKey, onPrint, onExportPdf
}: any) => (
...
const ExportModuleCard = ({ mod, scope, styles, exportingKey, onExport }: any) => (
    <div className={`rounded-2xl border bg-white p-5 shadow-sm ${styles.card}`}>
        <div className="flex items-start gap-3">
            <div className={`rounded-xl bg-gray-50 p-3 ${styles.icon}`}>
                <FileText size={18} />
            ...
            <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 text-xs text-gray-500">{mod.note}</p>
            </div>
        </div>

        {scope === 'department' && typeof mod.onDateFilterChange === 'function' && (
            <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 px-3 py-3">
                <label htmlFor={`${mod.id}-date-filter`} className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                    Interview Date
                </label>
                <input
                    id={`${mod.id}-date-filter`}
                    type="date"
                    value={mod.dateFilterValue || ''}
                    onChange={(event) => mod.onDateFilterChange(event.target.value)}
                    className="mt-2 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none focus:border-gray-300"
                />
            </div>
        )}
        ...
        <div className="mt-5 flex flex-wrap gap-3">
            <button
                type="button"
                onClick={() => void onExport(mod.id, 'csv', mod.getPayload)}
                disabled={exportingKey !== null}
                className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold text-gray-700 disabled:opacity-60 ${styles.button}`}
            >
                {exportingKey === `${mod.id}-csv`
                    ? <Loader2 size={16} className="animate-spin" />
                    : <Download size={16} />}
            ...
            <button
                type="button"
                onClick={() => void onExport(mod.id, 'pdf', mod.getPayload)}
                disabled={exportingKey !== null}
                className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-60 ${styles.active}`}
            >
                {exportingKey === `${mod.id}-pdf`
                    ? <Loader2 size={16} className="animate-spin" />
                    : <FileText size={16} />}
                <span>Export PDF</span>
            </button>
...
const StaffExportCenterPage = ({
    ...
    const handlePrintMasterList = useCallback(async () => {
        ...
        try {
            const payload = await getPrintableMasterListPayload(departmentName, departmentDateFilters.printableMasterList, masterListPanelFilter);
            if (!payload.rows.length) {
    ...
    }, [departmentDateFilters.printableMasterList, departmentName, masterListPanelFilter, notify]);
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {scope === 'department' && (
                <PrintableMasterListCard
                    styles={styles}
                    dateValue={departmentDateFilters.printableMasterList}
                    onDateChange={(value: string) => setDepartmentDateFilter('printableMasterList', value)}
                    panelValue={masterListPanelFilter}
                    onPanelChange={setMasterListPanelFilter}
                    panelOptions={masterListPanelOptions}
                    exportingKey={exportingKey}
                    onPrint={() => void handlePrintMasterList()}
                    onExportPdf={() => void handleExport('printable-master-list', 'pdf', () => getPrintableMasterListPayload(departmentName, departmentDateFilters.printableMasterList, masterListPanelFilter))}
                />
            )}
