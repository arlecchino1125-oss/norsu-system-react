                        ...
                        {(() => {
                            ...
                            return nextActivity ? (
                                <div className="student-dashboard-event-preview border border-purple-100/50 rounded-2xl p-4 bg-gradient-to-br from-white to-purple-50/50 sm:p-5">
                                    ...
                                    <button onClick={() => setActiveView('events')} className={`w-full mt-4 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all btn-press ${attendanceMap[nextActivity.id]?.time_in ? 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-gradient-to-r from-slate-800 to-slate-900 text-white shadow-lg shadow-slate-500/20'}`}>{attendanceMap[nextActivity.id]?.time_in ? 'Checked In' : 'View Event'}</button>
                                </div>
