            ...
            >
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-purple-300">
                                Counseling Evaluation Intake
                            </p>
                            <h3 className="mt-1 text-lg font-black leading-tight text-white">{form?.title || 'Counseling Service Evaluation'}</h3>
                            {form?.description && (
                                ...
                                return (
                                    <article key={question.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                                        <div className="flex items-start gap-3">
                                            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-purple-100 text-xs font-black text-purple-700">{index + 1}</span>
                                        ...
                                        {question.question_type === 'choice' && (
                                            <div className="mt-3 space-y-2 sm:ml-10">
                                                {(question.choices ?? []).map((choice: string) => (
                                                    <label key={choice} className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700 cursor-pointer hover:bg-slate-50">
                                                        <input
                                                            type="radio"
                ...
                <div className="shrink-0 border-t border-slate-100 bg-white p-4 sm:px-5">
                    ...
                    <button
                        ...
                        className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-4 py-3 text-sm font-black text-white transition hover:bg-purple-500 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                        {isSubmitting ? 'Submitting...' : 'Submit Counseling Evaluation'}
                    </button>
                </div>
            </div>
        </div>,
