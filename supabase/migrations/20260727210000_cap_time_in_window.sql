-- Time-in now closes at whichever comes FIRST: the event ending, or 3h after it
-- started. The previous rule took whichever came LAST, which meant a 13h event
-- accepted time-ins right up to the closing minute.
--
--   8am - 9am   -> closes 09:00 (the end comes first)
--   8am - 12pm  -> closes 11:00 (the 3h cap comes first)
--   8am - 9pm   -> closes 11:00 (the 3h cap comes first)
--
-- Keep in lockstep with src/utils/eventWindows.ts, which uses Math.min for the
-- same reason. src/lib/eventWindowMigrationLockstep.test.ts fails if they part.
...
BEGIN
    ...
    RETURN QUERY SELECT
        v_start,
        v_end,
        -- Time-in shuts at the end, or 3h in, whichever lands first.
        least(v_end, v_start + interval '3 hours'),
        -- Retained for signature compatibility only. Time-out has no closing
        -- window; nothing reads this column.
        NULL::timestamptz;
END;
$$;

-- Only the check-in close changes; every other rule in this function is
-- unchanged from 20260727190000_align_event_action_windows.sql.
...
BEGIN
    ...
    -- Check-in shuts at the end, or 3h in, whichever lands first.
    v_check_in_close_at := least(v_event_end_at, v_event_start_at + interval '3 hours');
