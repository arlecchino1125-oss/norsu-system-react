-- Students may read their own attendance and registrations, but all writes
-- must go through the controlled RPCs that derive identity from auth.uid().

DROP POLICY IF EXISTS event_attendance_student_insert_own ON public.event_attendance;
DROP POLICY IF EXISTS event_attendance_student_update_own ON public.event_attendance;
DROP POLICY IF EXISTS event_registrations_student_insert_own ON public.event_registrations;
DROP POLICY IF EXISTS event_registrations_student_update_own ON public.event_registrations;
...
REVOKE EXECUTE ON FUNCTION public.record_student_event_attendance(
    bigint,
    text,
    text,
    double precision,
...
GRANT EXECUTE ON FUNCTION public.record_student_event_attendance(
    bigint,
    text,
    text,
    double precision,
