...
const resolveStudentLogin = async (adminClient: any, studentId: unknown) => {
    const nextStudentId = String(studentId || '').trim();
    if (!nextStudentId) {
        return json({ success: false, error: 'Student ID is required.' }, 400);
    }
...
serve(async (request) => {
    ...
    try {
        ...
        if (mode === 'resolve-student-login') {
            return await resolveStudentLogin(adminClient, body.studentId);
        }
