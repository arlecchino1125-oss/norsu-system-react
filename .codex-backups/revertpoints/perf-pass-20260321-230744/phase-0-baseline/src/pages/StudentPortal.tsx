...
export default function StudentPortal() {
    ...
    const invokeManagedStudentFunction = useCallback(async (body: any) => {
        ...
        const { data, error, response } = await supabaseClient.functions.invoke('manage-student-accounts', {
            body,
        ...
        if (error) {
            ...
            if (String(error.message || '').includes('non-2xx')) {
                throw new Error(detailedMessage || 'Your student session could not be verified. Please sign in again.');
            }
            throw new Error(detailedMessage || error.message || 'Failed to update your student profile.');
        }

        if (!data?.success) {
            throw new Error(data?.error || 'Failed to update your student profile.');
        }
    ...
    const syncStudentAuthEmailIfNeeded = useCallback(async (nextEmailValue: unknown) => {
        ...
        const { data, error } = await supabaseClient.functions.invoke('manage-student-accounts', {
            body: {
        ...
        if (error) {
            if (String(error.message || '').includes('non-2xx')) {
                throw new Error('Your Supabase login session could not be verified. Please sign out and sign in again, then try again.');
            }
            throw new Error(error.message || 'Failed to sync your student login email.');
        }

        if (!data?.success) {
            throw new Error(data?.error || 'Failed to sync your student login email.');
        }
    ...
    const saveStudentSecurityEmail = async (nextEmailValue: unknown) => {
        if (!personalInfo.studentId) {
    ...
    const updateStudentPassword = async (nextPasswordValue: unknown) => {
