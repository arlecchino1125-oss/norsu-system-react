...
const SupportRequestsPage = ({ functions }: SupportRequestsPageProps) => {
    ...
    const invokeManagedCareServicesFunction = async (body: any) => {
        ...
        const { data, error, response } = await supabase.functions.invoke('manage-care-services', {
            body,
        ...
        if (error) {
            ...
            if (String(error.message || '').includes('non-2xx')) {
                throw new Error(detailedMessage || 'Your CARE Staff session could not be verified. Please sign in again.');
            }
            throw new Error(detailedMessage || error.message || 'Failed to manage CARE services.');
        }

        if (!data?.success) {
            throw new Error(data?.error || 'Failed to manage CARE services.');
        }
    ...
    const handleForwardSupport = async () => {
        ...
        try {
            ...
            await invokeManagedCareServicesFunction({
