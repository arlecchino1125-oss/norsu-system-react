...
const NATPortal = () => {
    ...
    const [activationSuccess, setActivationSuccess] = useState<{ visible: boolean; studentId: string; course: string; emailSent: boolean }>({ visible: false, studentId: '', course: '', emailSent: true });
    ...
    const invokeNatManagementFunction = async (body: any, fallbackMessage: string) => {
        const { data, error, response } = await supabase.functions.invoke('manage-nat-applications', {
            body,
        ...
        if (error) {
            ...
            if (response) {
                try {
                    ...
                    // Ignore unreadable error payloads and fall back to the default message.
                }
            }
    ...
    const handleActivationSuccessContinue = () => {
        setActivationSuccess({ visible: false, studentId: '', course: '', emailSent: true });
        setCurrentUser(null);
        setCredentials(null);
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 pt-2">
                                        ...
                                        <button
                                            ...
                                            className="py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 hover:-translate-y-1 transition-all"
                                        >Confirm</button>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <p className="text-sm text-gray-600 leading-relaxed">
                                        {activationSuccess.emailSent
                                            ...
                                            : 'Your account was activated, but the confirmation email could not be sent. You can still continue to the student login page and use your NAT password.'}
                                    </p>
                                    <button
                                        type="button"
