...
export default function DeptDashboard() {
    ...
    const invokeManagedAdmissionsFunction = useCallback(async (body: any) => {
        ...
        if (!accessToken) {
            throw new Error('Your login session has expired. Please sign in again and try once more.');
        }

        const { data, error, response } = await supabase.functions.invoke('manage-department-admissions', {
            body,
        ...
        if (error) {
            ...
            if (String(error.message || '').includes('non-2xx')) {
                throw new Error(detailedMessage || 'Your department session could not be verified. Please sign in again.');
            }
            throw new Error(detailedMessage || error.message || 'Failed to manage department admissions.');
        }

        if (!data?.success) {
            throw new Error(data?.error || 'Failed to manage department admissions.');
        }

        return data;
    }, []);

    const invokeApplicantEmailFunction = useCallback(async (body: any) => {
        const { data, error, response } = await supabase.functions.invoke('send-email', {
            body,
        ...
        if (error) {
            const detailedMessage = await readFunctionErrorMessage(response || error?.context);
            if (String(error.message || '').includes('non-2xx')) {
                throw new Error(detailedMessage || 'Failed to send applicant email.');
            }
            throw new Error(detailedMessage || error.message || 'Failed to send applicant email.');
        }

        if (data?.success === false) {
            throw new Error(data?.error || 'Failed to send applicant email.');
        }
    ...
    const sendAdmissionsEmailNotification = useCallback(async (payload: any) => {
        ...
        try {
            await invokeApplicantEmailFunction(payload);
            return {
                emailSent: true,
                emailError: null
            };
            ...
            return {
                emailSent: false,
                emailError: error?.message || 'Failed to send applicant email.'
            };
        }
    }, [invokeApplicantEmailFunction]);
    ...
    const invokeManagedDepartmentServicesFunction = useCallback(async (body: any) => {
        ...
        const { data, error, response } = await supabase.functions.invoke('manage-department-services', {
            body,
            headers: buildEdgeFunctionHeaders(accessToken)
        });
        ...
        if (error) {
            ...
            throw new Error(detailedMessage || error.message || 'Failed to manage department services.');
        }

        if (!data?.success) {
            throw new Error(data?.error || 'Failed to manage department services.');
        }
    ...
    const handleApproveAndSchedule = async (e: any) => {
        ...
        try {
            await invokeManagedDepartmentServicesFunction({
    ...
    const confirmApplicantSchedule = async (e: any) => {
        ...
        try {
            ...
            const emailPayload = {
                type: 'APPLICANT_INTERVIEW_SCHEDULED',
                email: selectedApplicant?.email,
                name: getApplicantFullName(selectedApplicant),
                referenceId: selectedApplicant?.reference_id,
                course: getCourseNameForChoice(selectedApplicant),
                interviewDate: `${applicantScheduleData.date} ${applicantScheduleData.time}`,
            ...
            patchAdmissionRows((rows) => rows.map((row: any) => (
                String(row.id) === String(selectedApplicant?.id)
                    ? {
                        ...row,
            ...
            void sendAdmissionsEmailNotification(emailPayload).then((emailResult) => {
                if (emailResult.emailSent === false) {
                    showToastMessage(`Interview scheduled, but applicant email failed: ${emailResult.emailError || 'Unknown email error.'}`, 'error');
                }
            });
        } catch (err: any) {
    ...
    return (
        <div className="flex h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 text-gray-800 font-sans overflow-hidden">
            ...
            {renderDeptModals({
                ...
                showApplicantScheduleModal, setShowApplicantScheduleModal, applicantScheduleData, setApplicantScheduleData, confirmApplicantSchedule, isSchedulingApplicant,
