...
import { Trash2, AlertTriangle, AlertCircle, CheckCircle, Plus, Link2 } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const [linkingAccountId, setLinkingAccountId] = useState<any>(null);
    ...
    const handleCreate = async (e: any) => {
        ...
        try {
            ...
            const { error: legacyError } = await supabase.from('staff_accounts').insert([payload]);
            if (legacyError) {
                showToast(legacyError.message, 'error');
            } else {
                showToast('Account created in legacy mode. Deploy provision-staff-account to auto-link new staff accounts.');
                setForm({ username: '', password: '', full_name: '', role: 'Department Head', department: '', email: '' });
                refetchAccounts();
            }
        } finally {
    ...
    const handleLink = async (account: any) => {
        if (account.auth_user_id) {
            showToast('This account is already linked.');
            return;
        }

        const roleLabel = account.role === 'Department Head' ? 'dean' : account.role.toLowerCase();
        if (!confirm(`Link this legacy ${roleLabel} account to Supabase Auth using its current username and password?`)) {
            return;
        }

        setLinkingAccountId(account.id);
        try {
            const data = await invokeManagedStaffFunction({
                mode: 'link-account',
                staffAccountId: account.id
            });

            const suffix = account.id === session.id
                ? ' Log out and sign back in next time to start using the linked auth login.'
                : '';
            showToast(
                data?.alreadyLinked
                    ? `Account is already linked.${suffix}`
                    : `Legacy account linked to Supabase Auth.${suffix}`
            );
            refetchAccounts();
        } catch (error: any) {
            if (isFunctionUnavailableError(error?.message || '', error?.status, error?.errorName)) {
                showToast('Linking blocked: deploy the latest manage-staff-accounts function first.', 'error');
                return;
            }

            showToast(error.message || 'Failed to link account.', 'error');
        } finally {
                    ...
                    <div className="lg:col-span-2 bg-white rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-gray-50 border-b"><tr><th className="p-4">Name</th><th className="p-4">Role</th><th className="p-4">Details</th><th className="p-4">Username</th><th className="p-4">Auth</th><th className="p-4">Password</th><th className="p-4 text-right">Action</th></tr></thead>
                                        ...
                                        <tr key={acc.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-1 rounded text-xs font-bold ${acc.auth_user_id ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                                                    {acc.auth_user_id ? 'Linked' : 'Legacy'}
                                                </span>
                                            </td>
                                                ...
                                                <div className="inline-flex items-center gap-3">
                                                    {!acc.auth_user_id && (
                                                        <button
                                                            ...
                                                            title="Link this legacy account to Supabase Auth"
                                                        >
