...
export default function StudentPortal() {
    ...
    useEffect(() => {
        const checkSession = async () => {
            ...
            try {
                ...
                studentQuery = authUserId
                    ? studentQuery.eq('auth_user_id', authUserId)
                    : studentQuery.eq('student_id', studentId);

                const { data: studentRows, error: studentError } = await studentQuery;
                let studentData: any = Array.isArray(studentRows) ? studentRows[0] : null;

                if (!studentData && authUserId && studentId) {
                    const { data: fallbackStudent, error: fallbackError } = await supabaseClient
                        .from('students')
                        .select(STUDENT_LIST_COLUMNS)
                        .eq('student_id', studentId)
                        .maybeSingle();
                    if (fallbackError) throw fallbackError;
                    studentData = fallbackStudent as any;
                }
    ...
    useEffect(() => {
        ...
        const counselingChannel = supabaseClient
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'counseling_requests', filter: `student_id=eq.${personalInfo.studentId}` }, (payload: any) => {
                if (activeView === 'dashboard' || activeView === 'counseling') {
                    refreshCounselingRequests();
                }
