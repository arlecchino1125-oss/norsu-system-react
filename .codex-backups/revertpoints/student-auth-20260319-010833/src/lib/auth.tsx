...
import { buildStudentAuthEmail, sanitizeStaffSession, sanitizeStudentSession } from './studentAuth';
...
export function AuthProvider({ children }: any) {
    ...
    React.useEffect(() => {
        ...
        const initializeSession = async () => {
            ...
            try {
                if (parsedSession && parsedSession.userType !== 'student') {
                    if (isActive) {
                ...
                if (data.session?.user) {
                    await restoreStudentSessionFromAuth(data.session.user);
                } else if (parsedSession?.userType === 'student') {
                    persistSession(null);
        ...
        const {
            ...
            if (currentSession?.userType === 'staff') {
                return;
            }

            if (nextAuthSession?.user) {
                restoreStudentSessionFromAuth(nextAuthSession.user).catch((error) => {
                    console.error('Failed to restore student session from Supabase Auth.', error);
                });
                return;
            }

            if (!currentSession || currentSession.userType === 'student') {
                persistSession(null);
            }
        });
    ...
    }, [persistSession, restoreStudentSessionFromAuth]);

    const logout = useCallback(() => {
        if (sessionRef.current?.userType === 'student') {
            supabase.auth.signOut().catch((error) => {
                console.error('Failed to sign out Supabase student session.', error);
            });
        }
        persistSession(null);
    }, [persistSession]);
