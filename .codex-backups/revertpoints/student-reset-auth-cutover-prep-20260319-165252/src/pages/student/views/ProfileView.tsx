...
const INPUT_CLASS = 'w-full appearance-auto rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm outline-none transition-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20';
    ...
    return (
        <div className={colSpan ? `col-span-${colSpan}` : ''}>
            {showsEditableControl ? (
                <label htmlFor={fieldId} className="block text-[10px] text-gray-400 uppercase font-bold mb-1">{label}</label>
            ) : (
                <p className="block text-[10px] text-gray-400 uppercase font-bold mb-1">{label}</p>
            )}
            {showsEditableControl ? (
                type === 'select' ? (
                    ...
                    <textarea id={fieldId} name={field} autoComplete="off" className={`${INPUT_CLASS} resize-none`} style={{ colorScheme: 'light' }} rows={3} value={activePersonalInfo[field] || ''} onChange={(e) => setDraftPersonalInfo((prev: any) => ({ ...prev, [field]: e.target.value }))} />
                ) : type === 'boolean' ? (
...
const Section = ({ icon, gradient, title, children, cardClass }: any) => (
    <div className={cardClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-3">{children}</div>
    </div>
);
...
function ProfileViewContent(p: any) {
    ...
    const profileCardClass = isEditing
        ? 'bg-white rounded-xl border border-blue-100/50 p-6 shadow-sm'
        : 'bg-white/90 backdrop-blur-sm rounded-xl border border-blue-100/50 p-6 shadow-sm card-hover';
    const profileSummaryCardClass = isEditing
        ? 'bg-white rounded-2xl border border-blue-100/50 p-6 shadow-sm'
        : 'bg-white/90 backdrop-blur-sm rounded-2xl border border-blue-100/50 p-6 shadow-sm card-hover';
    ...
    return (
        <div className={`flex flex-col lg:flex-row gap-6 lg:gap-8 ${isEditing ? 'student-profile-edit' : 'page-transition'}`} style={isEditing ? { colorScheme: 'light' } : undefined}>
            <div className={`w-full lg:w-80 lg:shrink-0 space-y-6 ${isEditing ? '' : 'animate-fade-in-up'}`}>
                <div className="bg-gradient-to-b from-blue-600 via-blue-700 to-blue-800 rounded-2xl overflow-hidden shadow-2xl shadow-blue-500/20 text-white text-center relative">
                    ...
                    <div className="px-8 pb-8 -mt-12 relative z-10">
                        ...
                        <button onClick={() => { setProfileTab('personal'); setIsEditing(true); }} className="w-full mt-6 lg:mt-8 bg-white/15 backdrop-blur-sm text-white py-2.5 rounded-xl text-xs font-bold hover:bg-white/25 transition-all border border-white/20 btn-press">Edit Profile</button>
                    </div>
                </div>
            ...
            <div className="flex-1 space-y-6">
                <div className={`bg-white/90 backdrop-blur-sm rounded-xl border border-blue-100/50 flex shadow-sm p-1 ${isEditing ? '' : 'animate-fade-in-up'}`} style={{ animationDelay: '100ms' }}>
                    {[{ id: 'personal', label: 'Personal Info', icon: <Icons.Profile /> }, { id: 'engagement', label: 'Engagement', icon: <Icons.Clock /> }, { id: 'security', label: 'Security', icon: <Icons.Support /> }].map((tab: any) => (
                        <button key={tab.id} onClick={() => { setProfileTab(tab.id); setIsEditing(false); }} className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2.5 rounded-lg text-[10px] sm:text-xs font-bold transition-all ${profileTab === tab.id ? 'bg-gradient-to-r from-blue-500 to-sky-400 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-900 hover:bg-purple-50'}`}>{tab.icon} <span className="hidden sm:inline">{tab.label}</span></button>
                    ))}
                </div>
                {profileTab === 'personal' && (
                    <div className={`space-y-6 ${isEditing ? '' : 'animate-fade-in-up'}`} style={{ animationDelay: '200ms' }}>
                        ...
                        <Section cardClass={profileCardClass} icon={<Icons.Events />} gradient="from-emerald-400 to-teal-500" title="Contact & Address">
                            ...
                            <Field {...fp} label="Current Residence" field="address" readOnly />
                        </Section>
                        ...
                        {isEditing && (
                            <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
                                <button onClick={() => setIsEditing(false)} className="px-6 py-2.5 bg-white/80 border border-purple-100/50 rounded-xl text-sm font-bold hover:bg-gray-50 transition-all">Cancel</button>
                                <button onClick={() => { setPersonalInfo(draftPersonalInfo); saveProfileChanges(draftPersonalInfo); }} className="px-6 py-2.5 bg-gradient-to-r from-blue-500 to-sky-400 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-500/20 hover:shadow-xl btn-press transition-all">Save Changes</button>
                            </div>
                        )}
                    </div>
                )}
