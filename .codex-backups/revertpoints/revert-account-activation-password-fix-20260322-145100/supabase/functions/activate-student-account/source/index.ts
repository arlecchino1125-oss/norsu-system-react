...
const generatePassword = (length = 12) => {
    // Keep activation passwords email-safe and easy to retype.
...
const normalizeCreateAuthError = (error: any, contactEmail: string) => {
    ...
    if (message.includes('already been registered')
        ...
        return new Error(`A Supabase Auth account already exists for "${contactEmail}". Use a different email or reset the existing auth account first.`);
    }
...
const activateNatStudentAccount = async (adminClient: any, body: Record<string, unknown>) => {
    ...
    await ensureEnrollmentKey(
        ...
        course,
        contactEmail,
        allowEnrollmentCreate && String(application.status || '') === 'Approved for Enrollment'
    );
