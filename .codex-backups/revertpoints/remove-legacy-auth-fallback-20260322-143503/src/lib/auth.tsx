...
const LEGACY_STAFF_AUTH_DOMAIN = 'staff.norsu.local';
const LEGACY_STUDENT_AUTH_DOMAIN = 'students.norsu.local';
...
const buildLegacyStaffAuthEmail = (username: unknown) => {
    const normalizedUsername = String(username || '').trim().toLowerCase();
    return normalizedUsername ? `${normalizedUsername}@${LEGACY_STAFF_AUTH_DOMAIN}` : null;
};

const buildLegacyStudentAuthEmail = (studentId: unknown) => {
    const normalizedStudentId = String(studentId || '').trim();
    return normalizedStudentId ? `${normalizedStudentId}@${LEGACY_STUDENT_AUTH_DOMAIN}` : null;
};
...
const signInWithCandidateEmails = async (emails: Array<string | null | undefined>, password: string) => {
    const attemptedEmails = new Set<string>();
    let lastResult: { data: any; error: any; email: string } | null = null;

    for (const rawEmail of emails) {
        const candidateEmail = normalizeLoginEmail(rawEmail);
        if (!candidateEmail || attemptedEmails.has(candidateEmail)) {
            continue;
        }

        attemptedEmails.add(candidateEmail);
        const { data, error } = await signInWithResolvedEmail(candidateEmail, password);
        lastResult = { data, error, email: candidateEmail };
...
export function AuthProvider({ children }: any) {
    ...
    const loginStaff = useCallback(async (username: any, password: any, requiredRole: any) => {
        ...
        try {
            ...
            const authAttempt = await signInWithCandidateEmails([
                userCheck.email,
                buildLegacyStaffAuthEmail(trimmedUsername)
            ], password);
            ...
            if (!authAttempt) {
                ...
                return {
                    success: false,
                    error: 'This staff account has no valid auth login email yet.'
                };
            }

            const { data: authData, error: authError } = authAttempt;
    ...
    const loginStudent = useCallback(async (studentId: any, password: any) => {
        ...
        try {
            ...
            const authAttempt = await signInWithCandidateEmails([
                studentLookup.email,
                buildLegacyStudentAuthEmail(trimmedStudentId)
            ], password);
            ...
            if (!authAttempt) {
                ...
                return {
                    success: false,
                    error: 'This student account has no valid auth login email yet.'
                };
            }
