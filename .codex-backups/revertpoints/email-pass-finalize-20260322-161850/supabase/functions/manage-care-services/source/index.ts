...
const scheduleCounseling = async (
    ...
    await assertCareStaffRequest(adminClient, request);
...
const completeCounseling = async (
    ...
    await assertCareStaffRequest(adminClient, request);
    const nextRequestId = String(requestId || '').trim();
    const nextPublicNotes = String(publicNotes || '').trim();
    const nextPrivateNotes = String(privateNotes || '').trim();
...
const forwardSupportToDepartment = async (
    ...
    if (careNotes === undefined || careNotes === null || String(careNotes).trim() === '') {
...
const completeSupport = async (
    ...
    await assertCareStaffRequest(adminClient, request);
    const nextRequestId = String(requestId || '').trim();
    const nextResolutionNotes = String(resolutionNotes || '').trim();
