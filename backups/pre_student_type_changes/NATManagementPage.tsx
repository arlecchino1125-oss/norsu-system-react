...
import { getAdmissionSchedules, getApplicationsPage, getCoursesForNat } from '../../services/natService';
...
const NATManagementPage = ({ showToast }: any) => {
    ...
    }, [searchTerm, resultFilter, completedFilter, applicationsPage, completedPage, testTakersPage]);
    ...
    const fetchData = async () => {
        ...
        try {
            const [
                ...
                getApplicationsPage(
                    { mode: 'applications', search: searchTerm, status: resultFilter },
                ...
                getApplicationsPage(
                    { mode: 'test_takers', course: resultFilter },
                    { page: testTakersPage, pageSize: DEFAULT_PAGE_SIZE }
                ),
                getAdmissionSchedules(),
                getCoursesForNat(),
                supabase.from('applications').select('id, status, time_in, time_out, priority_course, test_date, test_time')
            ]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm"><p className="text-xs text-gray-500">Test Takers</p><p className="text-xl font-bold">{summaryApplications.filter(a => a.time_in && a.time_out).length}</p></div>
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm"><p className="text-xs text-gray-500">Passed</p><p className="text-xl font-bold">{summaryApplications.filter(a => a.status === 'Passed' || String(a.status || '').includes('Qualified for Interview')).length}</p></div>
            </div>
