...
export default function StudentPortal() {
    ...
    const toSexCode = (value: string | null | undefined) => {
    ...
    const toYearLevelCode = (value: string | null | undefined) => {
    ...
    const splitPersonName = (rawValue: string | null | undefined) => {
    ...
    const handleApplyScholarship = async (scholarship: any) => {
        ...
        try {
            ...
            const fatherName = splitPersonName(personalInfo.fatherName);
            const motherName = splitPersonName(personalInfo.motherName);
            const disabilityInfo = personalInfo.isPwd ? (personalInfo.pwdType || 'PWD') : null;
            const indigenousGroup = personalInfo.isIndigenous ? (personalInfo.indigenousGroup || null) : null;
            ...
            const snapshotPayload = {
                ...
                student_last_name: personalInfo.lastName || null,
                student_given_name: personalInfo.firstName || null,
                student_ext_name: personalInfo.suffix || null,
                student_middle_name: personalInfo.middleName || null,
                sex_code: toSexCode(personalInfo.sex || personalInfo.gender),
                birthdate: personalInfo.dob || null,
                complete_program_name: personalInfo.course || null,
                year_level_code: toYearLevelCode(personalInfo.year),
                father_last_name: fatherName.last || null,
                father_given_name: fatherName.given || null,
                father_middle_name: fatherName.middle || null,
                mother_last_name: motherName.last || null,
                mother_given_name: motherName.given || null,
                mother_middle_name: motherName.middle || null,
                permanent_street_barangay: personalInfo.street || personalInfo.address || null,
                permanent_zip_code: personalInfo.zipCode || null,
                disability_info: disabilityInfo,
                indigenous_people_group: indigenousGroup
            } as any;
            ...
            const fallbackPayloads = [
                ...
                {
                    ...basePayload,
                    course_year: snapshotPayload.course_year,
                    year_window_range: snapshotPayload.year_window_range
                },
                {
                    ...basePayload,
                    course_year: snapshotPayload.course_year
            ...
            for (const candidate of fallbackPayloads) {
                const { error } = await supabaseClient.from('scholarship_applications').insert([candidate]);
                if (!error) {
                    insertError = null;
                    break;
                }
                ...
                const schemaError = errorText.includes('does not exist')
    ...
    const submitCourseYearConfirmation = async () => {
        ...
        try {
            ...
            const storedSession = localStorage.getItem('norsu_session');
            if (storedSession) {
                try {
                    const parsed = JSON.parse(storedSession);
                    localStorage.setItem('norsu_session', JSON.stringify({
                        ...parsed,
                        course: courseYearGate.course,
                        year_level: courseYearGate.year,
                        status: 'Active',
                        course_year_update_required: false,
                        course_year_window_start: courseYearGate.windowStart || parsed.course_year_window_start || null,
                        course_year_window_end: courseYearGate.windowEnd || parsed.course_year_window_end || null,
                        course_year_confirmed_at: nowIso
                    }));
                } catch (storageError) {
                    console.warn('Failed to refresh local session cache after course/year confirmation.', storageError);
                }
            }
        ...
        const fetchAndSyncProfile = async () => {
            if (session && session.userType === 'student') {
                ...
                if (studentData.course_year_update_required) {
                    let trustedCourse = course || '';
                    let trustedYear = isValidYearLevel(studentData.year_level || '') ? studentData.year_level : '1st Year';
                ...
                if (storedSession) {
                    try {
                        ...
                        console.warn('Failed to refresh local student session cache.', storageError);
                    }
                }
            ...
            try {
                ...
                if (err.code === '23505') {
                    ...
                    const { data } = await supabaseClient
                        ...
                        .select('id, event_id, student_id, time_in, time_out, status')
